...
def get_last_five_operations(operation_with_list: list) -> list:
    sort_operations = sorted(operation_with_list, key=lambda operation: operation["date"], reverse=True)
    last_five_operations = sort_operations[0:5]
    return last_five_operations


def get_operations_formatted(last_five_operations: list):
    operations_formatted_list = []
    for operation in last_five_operations:
        date = datetime.strptime(operation["date"], "%Y-%m-%dT%H:%M:%S.%f").strftime(
            "%d.%m.%Y")
